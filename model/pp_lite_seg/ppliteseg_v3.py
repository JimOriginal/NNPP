...
import math
from typing import Iterator, Mapping, Optional, Tuple, Union, List, Dict, Any, Iterable
...
# from module_utils import ConvBNReLU, make_upsample_module
...
class STDCBackbone(AbstractSTDCBackbone):
    def __init__(
        ...
        in_channels: int = 3,
        out_down_ratios: Union[tuple, list] = (32,),#8,16,32
    ):
        """
...
class STDC2Backbone(STDCBackbone):
    def __init__(self, in_channels: int = 3, out_down_ratios: Union[tuple, list] = (32,)):
        super().__init__(
            ...
            stdc_steps=4,
            in_channels=in_channels,
            out_down_ratios=out_down_ratios,#(8,16,32)
        )
...
class PPM(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_out = ConvBNReLU(inter_channels*3, out_channels, kernel_size=3, padding=1, bias=False)
        self.conv_in = ConvBNReLU(in_channels, inter_channels, kernel_size=1, bias=False)
        self.conv_after_add = ConvBNReLU(inter_channels, out_channels, kernel_size=3, padding=1, bias=False)
        self.out_channels = out_channels
        self.upsample_mode = upsample_mode
        self.align_corners = align_corners
        self.pool_sizes = pool_sizes
    ...
    def forward(self, x):
        out = None#x 1024,8,8
        input_shape = x.shape[2:]
        x_add=self.conv_in(x)#128,8,8
        y1=self.branches[0](x)#512*1*1
        y1=F.interpolate(y1, size=input_shape, mode=self.upsample_mode, align_corners=self.align_corners)#512*8*8
        y1=y1+x_add
        y1=self.conv_after_add(y1)
        y2=self.branches[1](x)#2*2
        y2=F.interpolate(y2, size=input_shape, mode=self.upsample_mode, align_corners=self.align_corners)
        y2=y2+x_add
        y2=self.conv_after_add(y2)
        y3=self.branches[2](x)#4*4
        y3=F.interpolate(y3, size=input_shape, mode=self.upsample_mode, align_corners=self.align_corners)
        y3=y3+x_add
        y3=self.conv_after_add(y3)
        # y4=self.branches[3](x)#6*6
        # y4=F.interpolate(y4, size=input_shape, mode=self.upsample_mode, align_corners=self.align_corners)

        out_concat=torch.cat([y1,y2,y3],dim=1)
...
class UAFM(nn.Module):
    ...
    def forward(self, x, skip):
        ...
        atten = torch.cat([
            *self._min_max_spatial_reduce(x, use_concat=False),
            *self._min_max_spatial_reduce(skip, use_concat=False)
        ], dim=1)
        #x 512,8,8,skip 512,8,8
        #attn 4,8,8

        atten = self.conv_atten(atten)#1，8，8
    ...
    def _avg_max_spatial_reduce(x, use_concat: bool = False):  
        # test_x = torch.mean(x, dim=1, keepdim=True)
        # test_x2=torch.max(x, dim=1, keepdim=True)

        reduced = [
            torch.mean(x, dim=1, keepdim=True),
            torch.max(x, dim=1, keepdim=True)[0]#0表示max值，1表示max值的索引
        ]
        if use_concat:
    ...
    @staticmethod
    def _min_max_spatial_reduce(x, use_concat: bool = False):
        reduced = [
...
class PPLiteSegBase(SegmentationModule):
    ...
    def __init__(self,
        ...
        # jim 230424
        self.pe = GaussianRelativePE(256,sigma=20)
        ...
        context = SPPM(in_channels=backbone_out_channels[-1],
                       ...
                       align_corners=align_corners)
        # context = PPM(in_channels=backbone_out_channels[-1],
        #                inter_channels=sppm_inter_channels,
        #                out_channels=sppm_out_channels,
        #                pool_sizes=sppm_pool_sizes,
        #                upsample_mode=sppm_upsample_mode,
        #                align_corners=align_corners)
        #context = ASPP(in_channels=backbone_out_channels[-1],inter_channels=sppm_inter_channels,out_channels=sppm_out_channels)
        ...
        # self.mask_threshold = torch.nn.Parameter(torch.tensor([0.5]),requires_grad=True)
    ...
    def forward(self, x):
        ...
        if not self.use_aux_heads:
            ...
            # mask = torch.heaviside(x - self.mask_threshold, torch.tensor([1.], device=x.device, dtype=x.dtype))
            # mask = (self.mask_threshold + F.relu(x - self.mask_threshold))*map_scale
            # cost_pass = mask * map_scale  # b,h,w
            # cost_pass = torch.sum(cost_pass, dim=2)  # b,1
            # cost_pass = torch.sum(cost_pass, dim=2) # b,1
    ...
    #add mask_threshold to model parameters:
    def get_parameters(self, lr: float, training_params: HpmStruct) -> list:
        params = super().get_parameters(lr, training_params)
        params.append({"params": self.mask_threshold, "lr": lr})
        return params
...
#         super().__init__(num_classes=get_param(arch_params, "num_classes"),
#                          backbone=backbone,
#                          projection_channels_list=[256, 512, 512],
#                          sppm_inter_channels=512,
#                          sppm_out_channels=512,
#                          sppm_pool_sizes=[1, 2, 4],
#                          sppm_upsample_mode="bilinear",
#                          align_corners=False,
#                          decoder_up_factors=[1, 2, 2],
...
#                          dropout=get_param(arch_params, "dropout", 0.),
#                          use_aux_heads=get_param(arch_params, "use_aux_heads", False),
#                          aux_hidden_channels=[64, 128, 128],
#                          aux_scale_factors=[8, 16, 32])                         
                         

class PPLiteSegB_H(PPLiteSegBase):
    def __init__(self, arch_params: HpmStruct):
        backbone = STDC2Backbone(in_channels=get_param(arch_params, "in_channels", 3),
                                 out_down_ratios=[8, 16, 32])
        super().__init__(num_classes=get_param(arch_params, "num_classes"),
                         backbone=backbone,
                         projection_channels_list=[192, 512, 512],
                         sppm_inter_channels=512,
                         sppm_out_channels=512,
                         sppm_pool_sizes=[1, 2, 4],
                         sppm_upsample_mode="bilinear",
...
if __name__ == '__main__':
    ...
    data_sample = torch.randn(7, 3, 256, 256)
