...
if __name__ == '__main__':
    model = SwinUnet(window_size=8,num_classes=1)
    model.eval()
    import cv2
    import os
    import time
    ...
    import sys
    import numpy as np
    sys.path.append(r'/mnt/hdd1/jim_main_folder/projs/NNPP')
    ...
    sample=map_sample.MapSample.load("/mnt/hdd1/jim_main_folder/projs/NNPP/dataset/map_set_5k/train/MAP_001_1_0014.pt")
    map = sample.map#(256,256)
    map = map.unsqueeze(0).unsqueeze(0)
    map = torch.cat([map,map,map],dim=1)
    image = cv2.imread("/mnt/hdd1/jim_main_folder/projs/NNPP/dataset/map_set_png_1w/png/MAP_001_1_0019.png")
    tensor = np.transpose(image, (2, 0, 1)).reshape(1, 3, 256, 256)
    image_tensor = torch.from_numpy(np.transpose(image, (2, 0, 1)).reshape(1, 3, 256, 256))

    logits = model(image_tensor.float())
    print(logits.shape)

    # id = '{:02d}'.format(random.randint(0, 19))
    # pic_path = os.path.join("/mnt/hdd1/jim_main_folder/projs/NNPP/Swin-MAE-main","temp_"+id+"_.png")
    #draw_temp_pics(map.float(),pred, mask,model,pic_path)
